...
class SongScreen extends Component {
  ...
  animate() {
    ...
    Animated.timing(
      // see Animated API for more info, this just animates a value over time using an easing function
      this.animatedValue,
      {
        toValue: 1,
        duration: 90000,
...
HomeStackNav.navigationOptions = ({ navigation }) => {
  ...
  if (routeName === 'Song') {
    // gets rid of tab nav if playing song
  ...
  if (routeName === 'Home') {
    ...
    // placeholder for now to ensure when going to SongScreen that Home icon doesn't disappear
    navigationOptions.tabBarIcon = ({ focused }) => (
      <MaterialCommunityIcons name="home" size={25} color="black" />
    );
  }
...
StreamStackNav.navigationOptions = ({ navigation }) => {
  ...
  if (routeName === 'Stream') {
    navigationOptions.tabBarIcon = ({ focused }) => (
      <MaterialCommunityIcons name="flash" size={25} color="black" />
    );
  } else {
    // placeholder for now to ensure when going to SongScreen that Home icon doesn't disappear
    navigationOptions.tabBarIcon = ({ focused }) => (
